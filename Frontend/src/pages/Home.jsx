...
const handleSubmit = async (e) => {
  ...
  const API_KEY = import.meta.env.VITE_PLANTNET_API_KEY;
    const response = await fetch(
      `https://my-api.plantnet.org/v2/identify/all?api-key=${API_KEY}&nb-results=3&lang=en`,
...
{/* Prediction Display */}
{prediction && prediction.results && prediction.results[0] && (
  <div style={{
    ...
    <p style={{ margin: "0 0 6px 0" }}>
      <strong style={{fontSize: "1.1rem"}}>Prediction:</strong>{" "}
      <span style={{ color: "#16a34a" ,fontWeight: "700",fontSize: "1.3rem" }}>
        {prediction.results[0].species.scientificNameWithoutAuthor}
      </span>
    </p>
    {prediction.results[0].species.commonNames?.length > 0 && (
      <p style={{ margin: "0 0 6px 0" }}>
        <strong>Common name:</strong>{" "}
        {prediction.results[0].species.commonNames.join(", ")}
      </p>
    )}
    <p style={{ margin: "0" }}>
      <strong>Probability:</strong>{" "}
      {(prediction.results[0].score * 100).toFixed(1)}%
    </p>
  </div>
)}
        {/* Not a herb display */}
{prediction && prediction.notHerb && (
  <div style={{
    marginTop: "25px", padding: "15px",
  ...
  Prediction: <span style={{ color: "#dc2626" }}>Not a herb 🚫</span>
</p>
